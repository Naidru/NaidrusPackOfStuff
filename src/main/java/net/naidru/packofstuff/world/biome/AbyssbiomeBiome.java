...
import net.naidru.packofstuff.block.AbyssleavesBlock;
import net.naidru.packofstuff.block.AbyssgrassBlock;
import net.naidru.packofstuff.block.AbyssLogBlock;
...
public class AbyssbiomeBiome extends NaidruPackostuffModElements.ModElement {
	@ObjectHolder("naidru_packostuff:abyssbiome")
	public static final CustomBiome biome = null;
	public AbyssbiomeBiome(NaidruPackostuffModElements instance) {
		super(instance, 163);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(-10092544).waterFogColor(-10092544)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(AbyssgrassBlock.block.getDefaultState(),
							Blocks.STONE.getDefaultState(), Blocks.STONE.getDefaultState())));
			setRegistryName("abyssbiome");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(AbyssLogBlock.block.getDefaultState()),
									new SimpleBlockStateProvider(AbyssleavesBlock.block.getDefaultState()))).baseHeight(7)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == AbyssgrassBlock.block.getDefaultState().getBlock() || ground == Blocks.STONE.getDefaultState().getBlock())
							&& (ground2 == AbyssgrassBlock.block.getDefaultState().getBlock()
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == AbyssleavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, AbyssleavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, AbyssLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == AbyssLogBlock.block.getDefaultState().getBlock()
					|| blockType == AbyssleavesBlock.block.getDefaultState().getBlock()
					|| blockType == AbyssgrassBlock.block.getDefaultState().getBlock() || blockType == Blocks.STONE.getDefaultState().getBlock();
		}
