...
public class StonelandBiome extends NaidruPackostuffModElements.ModElement {
	@ObjectHolder("naidru_packostuff:stoneland")
	public static final CustomBiome biome = null;
	public StonelandBiome(NaidruPackostuffModElements instance) {
		super(instance, 5);
	}
		...
		public CustomBiome() {
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(Blocks.STONE.getDefaultState()),
									new SimpleBlockStateProvider(Blocks.STONE.getDefaultState()))).baseHeight(104)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.STONE.getDefaultState().getBlock() || ground == Blocks.STONE.getDefaultState().getBlock())
							&& (ground2 == Blocks.STONE.getDefaultState().getBlock() || ground2 == Blocks.STONE.getDefaultState().getBlock())))
						return false;
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												|| state.isIn(BlockTags.LEAVES) || state.getBlock() == Blocks.STONE.getDefaultState().getBlock()
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, Blocks.STONE.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												Blocks.GRANITE.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private void addVines(IWorld world, BlockPos pos, Set<BlockPos> changedBlocks, MutableBoundingBox bbox) {
			setTreeBlockState(changedBlocks, world, pos, Blocks.STONE.getDefaultState(), bbox);
			int i = 5;
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.STONE.getDefaultState().getBlock()
					|| blockType == Blocks.STONE.getDefaultState().getBlock() || blockType == Blocks.STONE.getDefaultState().getBlock()
